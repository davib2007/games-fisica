...
// A simple horizontal-patrolling enemy that reuses the spider sprite/animations
export class Bat extends CharacterGameObject {
    constructor(config: BatConfig) {
        ...
        // update visual orientation on direction change
        ...
        // start simple left-right patrol AI
    ...
    #changeDirection(): void {
        // reset controls and then pick left or right to simulate patrol
        this.controls.reset();

        this.scene.time.delayedCall(ENEMY_SPIDER_CHANGE_DIRECTION_DELAY_WAIT, () => {
            const goRight = Phaser.Math.Between(0, 1) === 1;
            if (goRight) {
